...
const getRolesporProyecto = (request, response) => {
  ...
  connection.query(
    "SELECT * FROM roles WHERE proyecto = ?",
    [id],
    (error, results) => {
      if (error) {
        console.log(error);
        return response.status(500).json({ error: "Error al obtener roles" });
      }
...
const agregarRoles = (request, response) => {
  ...
  connection.query(
    "INSERT INTO roles(nombre, proyecto) VALUES (?,?)",
    [nombre, proyecto],
    (error, results) => {
      if (error) {
        console.log(error);
        return response.status(500).json({ error: "Error al agregar rol" });
      }
...
const eliminarRoles = (request, response) => {
  ...
  connection.query(
    ...
    (error, results) => {
      if (error) {
        console.log(error);
        return response.status(500).json({ error: "Error al eliminar rol" });
      }
